...
buildFan <- function(hue = 0.5, saturation = 0.20, value = 0.55, alpha = 0.5,
                     ...
                     random.center = FALSE, segment.width = 8, n.segments = 200, depth = FALSE){
  ...
  rescale <- function(x){(x - min(x))/(max(x) - min(x)) * 2 - 1}
  x.end <- rescale(x.end)
  y.end <- rescale(y.end)
...
printFans <- function(hsv.array = seq(0, 1, length.out = 100), filename = "fans.png",
                      output.image.width = 4608, output.image.height = 2880, ...){
